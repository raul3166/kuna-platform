...
export class PurchaseOrderItemsService {
  ...
  await this.prisma.purchaseOrder.findUnique({
    where: {
      id: createPurchaseOrderItemDto.purchaseOrderId,
    },
  });
...
const product =
  await this.prisma.product.findUnique({
    where: {
      id: createPurchaseOrderItemDto.productId,
    },
  });
...
const subtotal =
  ...
  createPurchaseOrderItemDto.purchaseOrderId,
);
return item;
  }
  ...
  const purchaseOrderItem =
    await this.getPurchaseOrderItemOrThrow(id);
  ...
  if (
    ...
    const product =
      await this.prisma.product.findUnique({
        where: {
          id: updatePurchaseOrderItemDto.productId,
        },
      });
  ...
  return {
    message:
      'Purchase order item removed successfully',
  };
}
...
private async getPurchaseOrderItemOrThrow(
  id: string,
) {
  const purchaseOrderItem =
