...
export class GoodsReceiptsController {
  ...
  @ApiResponse({
    status: 404,
    description: 'Purchase order or organization not found.',
  })
  @ApiResponse({
    status: 409,
    description: 'Purchase order is not confirmed.',
  })
  ...
  @ApiOperation({
    summary: 'Process goods receipt and update stock',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 409,
    description: 'Goods receipt is not in DRAFT status or has no items.',
  })
  @Permissions('goods-receipts.process')
  @Post(':id/process')
  process(@Param('id') id: string) {
    return this.goodsReceiptsService.process(id);
  }
  ...
  @ApiResponse({
    status: 409,
    description: 'Only draft goods receipts can be modified.',
  })
